...
En este apartado veremos las distintas pruebas que he realizado, aunque sólo mencionaré las evoluciones más importantes debido al gran número de intentos que he realizado. Véase que el conjunto de imágenes de test siempre será de un total de 140 imágenes escogidas al azar que no tienen nada que ver con la base de datos empleada para el entrenamiento.
...
\subsubsection{Base de datos}
Para este primer intento he cogido simplemente las caras frontales de la base de datos, dejando a un lado las laterales (Figura \ref{fig:primerIntentoDBangles}).
\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{imagenes/primerIntentoDBangles}
\caption[KDEF primer intento]{Ángulos seleccionados de KDEF por cada sujeto. (Los ángulos de las imágenes en rojo se han ignorado.)}
...
Puesto que el problema aparente del escaso rendimiento del primer intento han sido el escaso número de imágenes y su recorte constante para todas las imágenes, en este segundo intento vamos a intentar subsanar eso.
\subsubsection{Base de datos}
He seleccionado no sólo las imágenes frontales, como en el primer intento, si no también las imágenes laterales y he ignorado las de perfil (Figura \ref{fig:segundoIntentoDB}). Esto supone un total de 2940 imágenes.
\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{imagenes/segundoIntentoDB}
\caption[KDEF segundo intento]{Ángulos seleccionados de KDEF por cada sujeto. (Los ángulos de las imágenes en rojo se han ignorado.)}
\label{fig:segundoIntentoDB}
\end{figure}
\subsubsection{Preprocesamiento}
En esta ocasión y a diferencia del primer intento, he intentado entrenar el clasificador, recreando las condiciones que se pueden presentar en una imagen de entrada de la aplicación. Tal y como se explicó en el Apartado \ref{sub:moduloClasificacion} , la aplicación usa un detector de caras para crear un recorte y este es el que se le da como entrada al clasificador. Por tanto, para cada una de las imágenes seleccionadas de la base de datos:
\begin{enumerate}
	\item Se crean dos nuevas imágenes aplicando un cambio de perspectiva. Para ello calcula la matriz de transformación entre el rectángulo original y en el que se quiere proyectar la imagen. Figura \ref{fig:segundoIntentoDA1}
		...
		\caption[Intento 2 Cambio de Perspectiva]{Cambio de perspectiva artificial. La imagen central es la imagen original y las otras dos se han generado de forma artificial.}
	...
	\item Se utiliza el mismo detector facial (Apartado \ref{sub:detectorFacial}) que usa la aplicación para obtener un recorte del rostro. En la imagen \ref{fig:segundoIntentoDA2} se puede ver una comparativa de ambos métodos.
		\begin{figure}[h]
			...
			\caption[Intento 2 Rectangulo vs Detector]{\textbf{Izquierda:} Recorte utilizando el detector facial. \textbf{Derecha:} Recorte usando el rectángulo del primer intento de entrenamiento.}
	...
	\item A partir de las imágenes anteriores, se rotan de forma aleatoria entre 10º y 20º para crear una nueva imagen rotada. Usamos el reflejo de los bordes en lugar de un valor constante para los casos en los que hay que rellenar la imagen rotada. De esta forma añadimos información de la propia imagen, aunque sea repetida. Puede verse en la imagen superior derecha de la Figura \ref{fig:segundoIntentoDA3}.
		\begin{figure}[h]
			...
			\caption[Intento 2 Rotaciones]{ Imágenes no rotodas a la izquierda. Rotaciones aleatorias a la derecha.}
	...
	\item Para cada una de las imágenes generadas en los puntos 1, 2 y 3, se modifica la intensidad del valor de los píxeles de forma que se suma o se resta una cantidad aleatoria, para generar dos nuevas imágenes, una más clara y otra más oscura. El valor aleatorio puede oscilar entre 25 y 65 unidades. Una muestra de esto se puede ver en la Figura \ref{fig:segundoIntentoDA4}.
		\begin{figure}[h]
			...
			\caption[Intento 2 Iluminación]{De izquierda a derecha: Imagen iluminada - Imagen original - Imagen oscurecida.}
	...
	\item A partir de las imágenes anteriores, creo nuevas aplicando zooms aleatorios, entre un 10\% y un 15\% más cerca. En la Figura \ref{fig:segundoIntentoDA5} se puede ver un ejemplo de esto.
		\begin{figure}[h]
			...
			\caption[Intento 2 Zooms]{Izquierda: Imagen con zoom aleatorio. Derecha: Imagen original.}
	...
	\item Por último, ante la situación de que la cara detectada fuese más pequeña que el tamaño de las imágenes que se clasifican, \(256\times256\), al reajustar su tamaño a esas mismas dimensiones, la imagen que recibe el clasificador estaría pixelada. Es por esto que también simulé pixelados reajustando cada una de las imágenes generadas anteriormente. Para ello, pasé cada una de las imágenes de tamaño \(256\times256\) a tamaños \(40\times40\) y \(80\times80\) obteniendo dos imágenes nuevas. Ahora de nuevo las reajusté a \(256\times256\) usando un método de interpolación aleatorio de entre todos los que ofrece OpenCV\cite{opencvResize}. El resultado es el que se puede ver en la Figura \ref{fig:segundoIntentoDA6}.
		\begin{figure}[h]
			...
			\caption[Intento 2 Pixelado]{De izquierda a derecha: \textbf{1ª} Imagen original - \textbf{2ª} Imagen reajustada de \([80\times80] \longrightarrow [256\times256]\) - \textbf{3ª} Imagen reajustada de \([40\times40] \longrightarrow [256\times256]\).}
...
\begin{figure}[h]
	...
	\caption[Intento 2 Muestra]{Muestra de imágenes de la base de datos tras las trasnformaciones.}
...
\begin{table}[h]
	...
	\end{tabular}
	\caption{DB empleada - Segundo intento}
...
En esta ocasión la precisión obtenida con el conjunto de validación es del 85\% frente al 87\% del primer intento. ¿Ha disminuido el rendimiento del clasificador?\\
Cuando clasificamos el mismo conjunto de test del primer intento con el clasificador entrenado en este segundo intento, obtenemos la matriz de confusión de la Tabla \ref{tab:entrenamiento2MC}.
...
Si comparamos esta tabla con la matriz de confusión del primer intento (Tabla \ref{tab:entrenamiento1MC}) la ganancia es más que notoria. En este caso la diagonal está mucho más llena de aciertos y el clasificador no erra la mayor parte del tiempo con los falsos positivos de la clase Afraid, como se vió en los resultados del primer entreno. Además podemos ver como el clasificador no sabe distinguir la expresión \textit{Angry} y \textit{Sad}, que son asociadas a la clase \textit{Disgusted}.\\
La mejoría en el resto de clases se traduce en un 44,29\% de acierto con el conjunto de test frente al 22,86\% del intento anterior, por lo que podemos decir que las técnicas de preprocesamiento han sido \textbf{positivas} en este caso.\\
\clearpage

\section{Tercer Intento}
Viendo la mejoría de rendimiento tras el proceso de pre-procesado del segundo intento, en este tercero intentaremos producir muchas más imágenes de la misma manera.
\subsubsection{Base de datos}
Para ello se han tenido en cuenta la misma batería de imágenes de la base de datos que en el segundo intento (Véase la Figura \ref{fig:segundoIntentoDB}). %Se desechan las imágenes de perfil porque el detector facial no es capaz de detectar caras de perfil.

\subsubsection{Preprocesamiento}
Se ha aplicado el mismo pre-procesado que en el segundo intento, pero para cada una de las transformaciones se han producido un mayor número de imágenes. Por ejemplo, a la hora de generar los pixelados, por cada imagen, se usan todos los métodos de interpolación disponibles en lugar de un aleatorio. La idea es intentar obtener aun más generalidad.\\
...
\begin{table}[h]
	...
	\end{tabular}
	\caption{DB empleada - Tercer intento}
...
El rendimiento de este clasificador con el conjunto de validación es del 86\% de acierto. Al clasificar el conjunto de test obtenemos un 51,43\% de acierto en este tercer intento frente al 44,29\% del primero. Un mejoría sustancial del 7,14\% pese a mi sospecha acerca del sobre-ajuste en el entrenamiento.
...
El clasificador va mejorando. En este nuevo intento voy a cambiar el método de pre-procesamiento que se ha usado en el segundo y tercer intento, porque como se ha visto en el tercero, aumentar el conjunto de imágenes de forma metódica no va a mejorar mucho más el rendimiento de clasificador.

\subsubsection{Base de datos}
Se escogerá la misma batería de imágenes de la base de datos que en el segundo y tercer intento (Véase la Figura \ref{fig:segundoIntentoDB}).
...
\begin{table}[h]
	...
	\end{tabular}
	\caption{DB empleada - Cuarto intento}
...
Este entrenamiento ha alcanzado un 82\% de precisión sobre el conjunto de validación. Al clasificar el conjunto de test he obtenido un 53,57\%, lo que supone un aumento del 2,14\% de precisión usando aproximadamente una sexta parte del número de imágenes que se usó en el tercer intento de entrenamiento. Por lo que podemos afirmar que esta técnica generaliza mejor que la de el tercer intento.\\
La matriz de confusión de este entrenamiento se muestra en la Tabla  \ref{tab:entrenamiento4MC} y es similar a la del tercer intento. Se mantienen las deficiencias y las fortalezas del entreno anterior. La clase Angry y Sad siguen siendo las peores.
...
\section{Quinto Intento}
Este quinto intento fue una prueba para intentar unir los puntos fuertes de los modelos entrenados en el tercer y cuarto intento. Para ello intenté, usando el conjunto de imágenes generado en el intento 4, entrenar un nuevo modelo, pero para la inicialización de la red usar los pesos de un modelo ya entrenado previamente. Más concretamente usé los pesos del modelo entrenado en el tercer intento. Esta técnica se conoce como Transfer Learning y fue introducida en el Apartado \ref{sub:funcionamientoCnn}.
...
Pese a todos los intentos, no he conseguido aumentar la precisión en expresiones como Angry y Sad, por lo que estoy pensando que quizás, las imágenes de estas dos clases de la base de datos no sean demasiado representativas de las imágenes de la misma clase en el conjunto de test.
\clearpage
